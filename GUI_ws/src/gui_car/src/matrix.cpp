...
Matrix Matrix::operator_triupper()
{
    for (int i = 0; i < rows_; ++i) {
        for (int j = 0; j < cols_; ++j) {
            if(i > j) {
...
Matrix Matrix::concat_by_col(Matrix A, Matrix B)
{
...
Matrix Matrix::concat_by_row(Matrix A, Matrix B)
{
    Matrix AB(A.rows_ + B.rows_, A.cols_);
    for (int i = 0; i < AB.rows_; ++i) {
        for (int j = 0; j < AB.cols_; ++j) {
            if (i < A.rows_)
                AB(i, j) = A(i, j);
            else
                AB(i, j) = B(i - A.rows_, j);
        }
    }
    return AB;
}
...
Matrix Matrix::inverse()
{
    ...
    Matrix AI = Matrix::concat_by_col(*this, I);
...
{
    *size = 0;
    // memset(row, 0 ,sizeof(row)/sizeof(c_int));
    // memset(col, 0 ,sizeof(col)/sizeof(c_int));
    // memset(val, 0 ,sizeof(val)/sizeof(double));
    for (int i = 0; i < m.size_cols(); i++)
        for (int j = 0; j < m.size_rows(); j++)
            if (abs(m.get_value(j,i)) > ESION_ZERO)
                (*size)++;

    // sparseMatrix
    // double **compactMatrix = new double*[3];
    // p[i] = new double[cols_];
    // row = new c_int[size];
    // col = new c_int[size];
    // val = new double[size];
    ...
    int k = 0;
    c_int* col_temp = new c_int[*size];
    for (int j = 0; j < m.size_cols(); j++)
        for (int i = 0; i < m.size_rows(); i++)
            if (abs(m.get_value(i,j)) > ESION_ZERO)
            {
                row[k] = i;
                col_temp[k] = j; // this value is not actual col
                val[k] = m.get_value(i,j);
                k++;
            }

    // create col value
    col[0] = 0; // first index is always 0
    c_int col_index = 0;
    ...
    col_index++;
    col[col_index] = *size; // final index is number of nonzero elements
